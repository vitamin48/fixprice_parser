...
переходит по ним, предварительно установив город и адрес магазина из констант CITY и ADDRESS (если не выбран режим
ручной установки города), считывает информацию и остатки каждого товара, если бренда товара нет в файле bad_brand.txt,
записывает результаты в файл JSON, который перезаписывается при новом старте программы (но не в случае перезапуска
браузера)

Из-за того, что товаров много, процесс такой:
1. Первая попытка парса, каждая ссылка записывается в Json.
2. Сколько попыток - столько и файлов Json (не забываем переименовывать, иначе - перезапись).
...
# ADDRESS = 'Брянск, ул.Красноармейская, д.93'
